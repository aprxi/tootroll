...
def list_by_partition(
    ...
    for pv in sorted(partition_values):
        ...
        parquet_files += list([
            f"{partition_key}={pv}/{pname}" for pname in
            os.listdir(pv_directory)
...
# def list_by_partition(server: str, database: str, partition_key: str) -> Optional[List[str]]:
#     directory = f"{DATABASE_DIR}/{server}/{database}"
#     if not os.path.isdir(directory):
#         return None
